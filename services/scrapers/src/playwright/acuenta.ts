/**
 * Scraper de Acuenta (acuenta.cl) — cadena low-cost de Walmart Chile.
 *
 * Patrón similar al de Líder (también Walmart). Productos detectados por
 * links de patrón /tienda/product/ o SKU numérico de 5+ dígitos.
 */
...
const BASE_URL = "https://www.acuenta.cl";
...
async function persistProducts(products: ExtractedProduct[]) {
  const chain = await prisma.chain.findUnique({ where: { slug: "acuenta" } });
  if (!chain) throw new Error("Cadena 'acuenta' no existe; corre el seed primero");
...
async function main() {
  ...
  console.log(`✓ acuenta: ${total} productos persistidos`);
  await browser.close();
