/**
 * Scraper de Salcobrand (salcobrand.cl).
 *
 * Salcobrand usa su propia plataforma e-commerce.
 * Detectamos productos por links a /producto/ o por SKU numérico.
 */
...
async function persistProducts(products: ExtractedProduct[]) {
  const chain = await prisma.chain.findUnique({ where: { slug: "salcobrand" } });
  if (!chain) throw new Error("Cadena 'salcobrand' no existe; corre el seed primero");
...
async function main() {
  ...
  console.log(`✓ salcobrand: ${total} productos persistidos`);
  await browser.close();
